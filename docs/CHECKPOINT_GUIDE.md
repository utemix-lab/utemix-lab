...
## System Fix (ритуал фиксации)

Лёгкая, единая фиксация состояния после заметного объёма работ.

### Когда делать
- после значимого блока изменений (архитектура, контракты, UI поведение)
- при накоплении “рассинхрона” между идеей и реализацией

### Что входит
- обновить `SYSTEM_OVERVIEW.md` (текущее состояние)
- при необходимости обновить `PROJECT_OVERVIEW.md` (концепт)
- создать checkpoint (обычно мини-ревизия)
- зафиксировать ADR (если было решение)
...
- понятный статус для внешней модели и новых участников
